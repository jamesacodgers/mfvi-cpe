...
path = os.path.join(os.getcwd(), "uci_data")
...
def _cache_path(key: str, target: Optional[str]) -> str:
    t = target if target is not None else "_default"
    t = re.sub(r"[^A-Za-z0-9_.-]+", "_", t)
    return os.path.join(CACHE_DIR, f"{CACHE_VERSION}__{key}__{t}.pkl")


def _try_load_cache(key: str, target: Optional[str]) -> Optional[Tuple[pd.DataFrame, pd.DataFrame]]:
    path = _cache_path(key, target)
    if os.path.exists(path):
        X, y = pd.read_pickle(path)
        return X, _ensure_single_col_df(y, "cached y")
    return None


def _save_cache(key: str, target: Optional[str], X: pd.DataFrame, y: pd.DataFrame) -> None:
    os.makedirs(CACHE_DIR, exist_ok=True)
    path = _cache_path(key, target)
    pd.to_pickle((X, y), path)
...
SPECS: dict[str, DatasetSpec] = {
    ...
    "kin8nm": DatasetSpec(default_target=None),
}
...
def load_dataset(key: str, *, target: Optional[str] = None) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if key == "kin8nm":
        if target is not None:
            raise ValueError("OpenML kin8nm exposes one default target; `target=` override is not supported here.")
        cached = _try_load_cache(key, None)
        ...
        X, y, *_ = ds.get_data(target=ds.default_target_attribute)
        y_df = _ensure_single_col_df(y, "openml y")
        _save_cache(key, None, X, y_df)
        return X, y_df

    chosen_target = target or spec.default_target
    cached = _try_load_cache(key, chosen_target)
    if cached is not None:
        return cached
    ...
    if fetch_ucirepo is not None and (spec.uci_id is not None or spec.uci_name is not None):
        try:
            ...
            if chosen_target is not None:
                if chosen_target in y_df.columns:
                    y_out = y_df[[chosen_target]]
                    _save_cache(key, chosen_target, X, y_out)
                    return X, y_out
                if y_df.shape[1] == 1:
                    y_out = _ensure_single_col_df(y_df, "uci y")
                    _save_cache(key, chosen_target, X, y_out)
                    return X, y_out
                raise KeyError(f"Requested target '{chosen_target}' not in ucimlrepo targets: {list(y_df.columns)}")
    ...
    if chosen_target is not None and chosen_target in df.columns:
        X, y = _split_xy(df, chosen_target)
        _save_cache(key, chosen_target, X, y)
        return X, y
